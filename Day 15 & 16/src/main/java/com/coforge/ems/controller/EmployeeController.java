...
@RequestMapping("api/v1/ems")
	...
	*/
	...
	//field injection is fast used for testing not proper testing
	@Autowired
	...
	public ResponseEntity<String> deleteByEid(@PathVariable("eid") int eid) {
		...
		try {
			...
			if(status) responseEntity = new ResponseEntity<>(environment.getProperty("ems.delete.success"),HttpStatus.OK);
		} catch (InvalidEmployeeObjectException e) {
	...
	public ResponseEntity<?> findByEid(@PathVariable("eid") int eid) {
		...
		try {
			...
			responseEntity = new ResponseEntity<>(environment.getProperty("ems.findid.failed"), HttpStatus.BAD_REQUEST);
	...
	public ResponseEntity<?> findByEname(@PathVariable String ename) {
		ResponseEntity<?> responseEntity = null;
	    List<Employee> employees;
		try {
			employees = service.findByEname(ename);
			responseEntity = new ResponseEntity<>(employees, HttpStatus.OK);
